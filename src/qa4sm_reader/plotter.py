...
class QA4SMPlotter:
    ...
    def _get_parts_name(Var, type='boxplot_basic') -> list:
        ...
        elif type in ['boxplot_tc', 'mapplot_basic']:
    ...
    def _get_legend_title(Var) -> str:
        ...
        dataset_ref = {Var.Datasets.ref_id:f"{Var.Datasets.ref["pretty_name"]} [{Var.Datasets.ref['mu'] if not scale_ds else scale_ds[1]["mu"]}]"}
        dataset_others = {Var.Datasets.others_id[i]:f"{Var.Datasets.others[i]["pretty_name"]} [{Var.Datasets.others[i]['mu'] if not scale_ds else scale_ds[1]["mu"]}]" for i in range(len(Var.Datasets.others))}
    ...
    def _boxplot_definition(self,
        ...
        fig, ax = QA4SMPlotter._append_legend_title(fig, ax, Var)
